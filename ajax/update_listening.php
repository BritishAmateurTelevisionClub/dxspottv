...
$got_variables = (isset($_REQUEST["70cm"]) && isset($_REQUEST["23cm"]));
...
if(mysqli_num_rows ($sessions_result)==0) { // session doesn't exist on server
	...
	if ($_COOKIE["session_key"]==$target_row["session_id"]) {
		...
		$l70cm = mysqli_real_escape_string($dbc, $_REQUEST["70cm"]);
		$l70cm_freq = mysqli_real_escape_string($dbc, $_REQUEST["70cm_freq"]);
		$l23cm = mysqli_real_escape_string($dbc, $_REQUEST["23cm"]);
		$l23cm_freq = mysqli_real_escape_string($dbc, $_REQUEST["23cm_freq"]);
		$l13cm = mysqli_real_escape_string($dbc, $_REQUEST["13cm"]);
		$l13cm_freq = mysqli_real_escape_string($dbc, $_REQUEST["13cm_freq"]);
