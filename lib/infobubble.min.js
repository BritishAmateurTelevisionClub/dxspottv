...
InfoBubble.prototype.buildDom_=function(){var bubble=this.bubble_=document.createElement("DIV");bubble.style["position"]="absolute";bubble.style["zIndex"]=this.baseZIndex_;var tabsContainer=this.tabsContainer_=document.createElement("DIV");tabsContainer.style["position"]="relative";tabsContainer.style["display"]="inline";var close=this.close_=document.createElement("IMG");close.style["position"]="absolute";close.style["width"]=this.px(12);close.style["height"]=this.px(12);close.style["border"]=0;
close.style["zIndex"]=this.baseZIndex_+1;close.style["cursor"]="pointer";close.src="https://maps.gstatic.com/intl/en_us/mapfiles/iw_close.gif";var that=this;google.maps.event.addDomListener(close,"click",function(){that.close();google.maps.event.trigger(that,"closeclick")});var contentContainer=this.contentContainer_=document.createElement("DIV");contentContainer.style["overflowX"]="auto";contentContainer.style["overflowY"]="auto";contentContainer.style["cursor"]="default";contentContainer.style["clear"]=
"both";contentContainer.style["position"]="relative";var content=this.content_=document.createElement("DIV");contentContainer.appendChild(content);var arrow=this.arrow_=document.createElement("DIV");arrow.style["position"]="relative";var arrowOuter=this.arrowOuter_=document.createElement("DIV");var arrowInner=this.arrowInner_=document.createElement("DIV");var arrowSize=this.getArrowSize_();arrowOuter.style["position"]=arrowInner.style["position"]="absolute";arrowOuter.style["left"]=arrowInner.style["left"]=
"50%";arrowOuter.style["height"]=arrowInner.style["height"]="0";arrowOuter.style["width"]=arrowInner.style["width"]="0";arrowOuter.style["marginLeft"]=this.px(-arrowSize);arrowOuter.style["borderWidth"]=this.px(arrowSize);arrowOuter.style["borderBottomWidth"]=0;var bubbleShadow=this.bubbleShadow_=document.createElement("DIV");bubbleShadow.style["position"]="absolute";bubble.style["display"]=bubbleShadow.style["display"]="none";bubble.appendChild(this.tabsContainer_);bubble.appendChild(close);bubble.appendChild(contentContainer);
arrow.appendChild(arrowOuter);arrow.appendChild(arrowInner);bubble.appendChild(arrow);var stylesheet=document.createElement("style");stylesheet.setAttribute("type","text/css");this.animationName_="_ibani_"+Math.round(Math.random()*1E4);var css="."+this.animationName_+"{-webkit-animation-name:"+this.animationName_+";-webkit-animation-duration:0.2s;"+"-webkit-animation-iteration-count:1;}"+"@-webkit-keyframes "+this.animationName_+" {from {"+"-webkit-transform: scale(0)}50% {-webkit-transform: scale(1.2)}90% "+
...
0;var maxHeight=this.get("maxHeight")||0;maxWidth=Math.min(mapWidth,maxWidth);maxHeight=Math.min(mapHeight,maxHeight);var tabWidth=0;if(this.tabs_.length)for(var i=0,tab;tab=this.tabs_[i];i++){var tabSize=this.getElementSize_(tab.tab,maxWidth,maxHeight);var contentSize=this.getElementSize_(tab.content,maxWidth,maxHeight);if(width<tabSize.width)width=tabSize.width;tabWidth+=tabSize.width;if(height<tabSize.height)height=tabSize.height;if(tabSize.height>tabHeight)tabHeight=tabSize.height;if(width<contentSize.width)width=
contentSize.width;if(height<contentSize.height)height=contentSize.height}else{var content=this.get("content");if(typeof content=="string")content=this.htmlToDocumentFragment_(content);if(content){var contentSize=this.getElementSize_(content,maxWidth,maxHeight);if(width<contentSize.width)width=contentSize.width;if(height<contentSize.height)height=contentSize.height}}if(maxWidth)width=Math.min(width,maxWidth);if(maxHeight)height=Math.min(height,maxHeight);width=Math.max(width,tabWidth);if(width==tabWidth)width=
width+2*padding;arrowSize=arrowSize*2;width=Math.max(width,arrowSize);if(width>mapWidth)width=mapWidth;if(height>mapHeight)height=mapHeight-tabHeight;if(this.tabsContainer_){this.tabHeight_=tabHeight;this.tabsContainer_.style["width"]=this.px(tabWidth)}this.contentContainer_.style["width"]=this.px(width);this.contentContainer_.style["height"]=this.px(height)};
